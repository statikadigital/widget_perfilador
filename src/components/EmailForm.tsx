...
const EmailForm: React.FC<EmailFormProps> = ({ 
  ...
  useEffect(() => {
    const emailInput = document.querySelector(`#${isMobile ? 'mobile' : 'desktop'}-email-input`) as HTMLInputElement;
    if (emailInput) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      const correoData = {
        ...
        fondo: portfolioName || '',
        clave: clave,
        esceConservador: String((simulationData as any)?.scenarioConservative || ''),
      };
