...
const SimulatorForm = ({ 
  ...
  const [duracionPlazo, setDuracionPlazo] = useState(0);
  const [period, setPeriod] = useState('anios');
  const [liquidez, setLiquidez] = useState('NO');
  
  const [inversionInicialValid, setInversionInicialValid] = useState(true);
  const [inversionMensualValid, setInversionMensualValid] = useState(true);
  
  const isInitialMount = useRef(true);
  const lastNotifiedData = useRef(null);
  const lastInitialData = useRef(null);
  ...
  useEffect(() => {
    ...
    const newPeriod = initialData.period === 'years' ? 'anios' : (initialData.period || 'anios');
  ...
  const handleInversionInicialFocus = (e) => {
    const num = moneyToNumber(e.target.value);
    e.target.value = num || '';
  };

  const handleInversionInicialInput = (e) => {
  ...
  const handleInversionMensualFocus = (e) => {
    const num = moneyToNumber(e.target.value);
    e.target.value = num || '';
  };

  const handleInversionMensualInput = (e) => {
    e.target.value = e.target.value.replace(/[^\d]/g, '');
  };
